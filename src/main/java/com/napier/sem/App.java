...
public class App {
    ...
    public Employee getEmployee(int ID) {
        try {
            ...
            if (rset.next()) {
                ...
                emp.last_name = rset.getString("title");
                emp.salary = rset.getInt("salary");
                //emp.debt = rset.getString("dept_name");
                emp.getManager();
    ...
    /**
     * Gets all the current employees and salaries by inputted role.
     *
     * @return A list of all employees in that role and salaries, or null if there is an error.
     */
    ...
    public ArrayList<Employee> getSalariesByDepartment(Department debt) {
        try {
            // Create an SQL statement
            Statement stmt = con.createStatement();
            // Create string for SQL statement
            String strSelect =
                    "SELECT employees.emp_no, employees.first_name, employees.last_name, salaries.salary "
                            ...
                            + "AND salaries.to_date = '9999-01-01' "
                            + "AND departments.dept_no = '" + debt + "'  ORDER BY employees.emp_no ASC";
            // Execute SQL statement
            ResultSet rset = stmt.executeQuery(strSelect);
            // Extract employee information
            ArrayList<Employee> employees = new ArrayList<Employee>();
